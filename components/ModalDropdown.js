...
export default class ModalDropdown extends Component {
  ...
  _renderItem = ({ item, index, separators }) => {
    const {accessible, renderItem} = this.props;
    return(
      <TouchableOpacity accessible={accessible} onPress={() => {this._handleItemAction(item, index, separators)}}>
        {
          renderItem ? renderItem(item, index, separators) : (
            <View style={{padding: 10, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#F4F4F4'}}>
              <Text style={{color: '#333', fontSize: 16, lineHeight: 18}}>{item}</Text>
            </View>
          )
        }
      </TouchableOpacity>
  ...
  _handleItemAction = (item, index, separators) => {
